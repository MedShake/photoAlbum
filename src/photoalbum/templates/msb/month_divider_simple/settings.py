...
class MonthDividerSimpleSettingsWidget(MsbTemplateSettingsWidget):
    ...
    def __init__(self,instance:PageInstance,photos,*,translator,render_service=None,page_format:PageFormat=A4,template_pack_settings=None,parent=None)->None:
        super().__init__(instance,photos,translator=translator,render_service=render_service,page_format=page_format,template_pack_settings=template_pack_settings,parent=parent); self._create_content(); self._load_state(); self._render_preview()
    def _create_content(self):
        ...
        right=QWidget(); rl=QVBoxLayout(right); rl.setContentsMargins(0,0,0,0); rl.setSpacing(8); rl.addWidget(self.create_preview_title()); self._preview=QLabel(); self._preview.setFixedSize(self.PREVIEW_WIDTH,self.PREVIEW_HEIGHT); self._preview.setAlignment(Qt.AlignmentFlag.AlignCenter); self._preview.setStyleSheet("border: 1px solid #888;background: white;"); rl.addWidget(self._preview,alignment=Qt.AlignmentFlag.AlignTop); root.addWidget(left,1); root.addWidget(right,0); self._font.currentIndexChanged.connect(self._changed); self._size.valueChanged.connect(self._changed)
