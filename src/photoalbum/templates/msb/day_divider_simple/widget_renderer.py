...
def day_title(page, translator) -> str:
    day = "1er" if translator.language == "fr" and page.day == 1 else str(page.day)
    return translator.tr(
        "day_divider.date", day=day,
        month=translator.month_name(page.month), year=page.year,
    )
...
class DayDividerSimpleWidgetRenderer:
    def paint(self, *, painter, instance, photos, target_rect, width, height,
              ...
              template_pack_settings=None, **kwargs) -> None:
        ...
        title = day_title(page, translator)
